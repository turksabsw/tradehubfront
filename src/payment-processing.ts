...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const displayOrderNumber = orderNumbers ? orderNumbers.split(',')[0] : `ORD-${Date.now().toString(36).toUpperCase()}`;
  ...
  function initOtpInputs() {
    ...
    verifyBtn?.addEventListener('click', () => {
      ...
      if (entered === otpCode) {
        ...
        render();
        setTimeout(() => {
          window.location.href = `/order-success.html?status=success&count=${count}&orderNumbers=${encodeURIComponent(orderNumbers)}`;
        }, 2000);
      } else {
  ...
  if (method === 'credit_card') {
    ...
    setTimeout(() => {
      window.location.href = `/order-success.html?status=pending&count=${count}&orderNumbers=${encodeURIComponent(orderNumbers)}`;
    }, 3000);
  }
});
