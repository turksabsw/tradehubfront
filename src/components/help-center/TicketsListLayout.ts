    ...
    <div class="bg-gray-50 min-h-screen" x-data="ticketsList()">
      <div class="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
        ...
        <div class="flex items-center justify-between mb-6">
          ...
          <a href="/pages/help/help-ticket-new.html" class="inline-flex items-center gap-2 px-4 py-2.5 bg-primary-500 text-white text-sm font-medium rounded-lg hover:bg-primary-600 transition-colors">
        ...
        <div class="mb-6">
          <div class="relative max-w-[400px]">
            ...
            <input type="text" x-model="searchQuery" placeholder="Talep no veya konu ara..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none">
          </div>
        </div>
              ...
              <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-shadow cursor-pointer" @click="toggleTicket(ticket.id)">
                <div class="flex items-center justify-between mb-2">
                  <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                    ...
                    x-text="ticket.status === 'open' ? 'A\u00e7\u0131k' : ticket.status === 'pending' ? 'Beklemede' : '\u00c7\u00f6z\u00fcld\u00fc'">
                  </span>
        ...
        <template x-if="filteredTickets.length === 0">
          <div class="bg-white border border-gray-200 rounded-lg p-12 text-center">
            ...
            <h3 class="text-base font-medium text-gray-700 mb-1">Destek talebi bulunamadı</h3>
            <p class="text-sm text-gray-400">Henüz destek talebi oluşturmadınız veya arama kriterlerinize uygun talep yok.</p>
          </div>
        </template>
        ...
        <template x-if="totalPages > 1">
          <div class="flex items-center justify-center gap-2 mt-8">
            <button @click="setPage(Math.max(1, currentPage - 1))" :disabled="currentPage === 1" class="px-3 py-1.5 text-sm border border-gray-300 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors cursor-pointer">Önceki</button>
            ...
            <button @click="setPage(Math.min(totalPages, currentPage + 1))" :disabled="currentPage === totalPages" class="px-3 py-1.5 text-sm border border-gray-300 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors cursor-pointer">Sonraki</button>
          </div>
        </template>
      </div>
    </div>
