...
export function OrdersSection(): string {
  ...
  return `
    <div class="orders bg-(--color-surface,#ffffff) rounded-(--radius-card) overflow-hidden transition-shadow duration-300 hover:shadow-[0_0_12px_0_rgba(0,0,0,0.12)]" data-orders-section>
      <div class="flex items-center justify-between px-[clamp(0.75rem,0.5rem+1vw,1.25rem)] pt-4 pb-3">
        ...
        <a href="#" class="orders__view-all text-[13px] text-(--color-text-placeholder,#999999) no-underline inline-flex items-center gap-0.5 transition-colors hover:text-(--color-cta-primary)" aria-label="Tüm siparişleri görüntüle">
          Tümünü görüntüle
