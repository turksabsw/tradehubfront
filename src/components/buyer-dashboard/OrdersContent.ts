...
export function OrdersContent(): string {
  return `
    <div class="orders__content mx-5 mb-5 mt-3 rounded-[var(--radius-card)] bg-(--color-orders-empty-bg) min-h-[180px] flex items-center justify-center" role="tabpanel" aria-label="Sipariş içeriği">
      <div class="flex flex-col items-center gap-3 py-8 px-5">
        ...
        <a href="#" class="orders__empty-btn inline-block py-2 px-6 rounded-[var(--radius-btn)] bg-(--color-orders-empty-btn-bg) text-[var(--color-orders-empty-btn-text)] border border-[var(--color-orders-empty-btn-border)] text-[13px] font-semibold no-underline transition-all hover:bg-(--color-surface-raised,#f5f5f5)" role="button">
