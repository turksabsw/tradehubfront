  ...
  return `
    <div x-data="messagesComponent" class="flex flex-col h-[calc(100vh-48px)] max-md:h-auto max-md:min-h-[calc(100vh-48px)] bg-(--color-surface,#ffffff) rounded-lg max-sm:rounded-none overflow-hidden" x-cloak>
      ...
      <div x-show="feedbackVisible" x-transition class="flex items-center justify-between px-5 py-2.5 max-sm:px-3 bg-gradient-to-br from-blue-500 to-blue-400 text-(--color-surface)">
        ...
        <button @click="dismissFeedback()" class="flex items-center justify-center w-6 h-6 border-none bg-transparent text-(--color-surface) cursor-pointer opacity-80 hover:opacity-100 transition-opacity" aria-label="Kapat">
