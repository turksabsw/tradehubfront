...
const STATUS_CONFIG = {
  open: { label: 'Açık', bg: 'bg-green-100 text-green-700' },
  pending: { label: 'Beklemede', bg: 'bg-amber-100 text-amber-700' },
  closed: { label: 'Çözüldü', bg: 'bg-gray-100 text-gray-600' },
};

const CATEGORY_ICONS: Record<string, string> = {
  'Sipariş': `<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>`,
  'Ödeme': `<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z"/></svg>`,
  'Kargo': `<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12"/></svg>`,
  ...
  return `
    <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-shadow cursor-pointer">
      ...
      <div class="flex items-center justify-between mb-2">
        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${status.bg}">${status.label}</span>
      ...
      <div class="flex items-start gap-2 mb-2">
        ...
        <div class="min-w-0">
          ...
          <span class="text-xs text-gray-500">#${ticket.id} &middot; ${ticket.category}</span>
        </div>
      </div>
      <!-- Bottom: last reply + snippet -->
      ${ticket.lastReply || ticket.snippet ? `
        <div class="pt-2 border-t border-gray-100">
          ${ticket.lastReply ? `<p class="text-xs text-gray-400 mb-1">Son yanıt: ${ticket.lastReply}</p>` : ''}
