...
export function StoreNav(data: StoreNavData): string {
  const menuItems = data.items.map(item => {
    if (item.dropdownType === 'products') {
      return `
        <li class="relative">
          <button class="store-nav__item store-nav__item--dropdown flex items-center gap-1 px-5 py-3 text-(--store-nav-text) text-[14px] font-normal cursor-pointer bg-transparent border-none transition-colors hover:bg-[rgba(255,255,255,0.1)]"
          ...
          <div class="store-nav__dropdown store-nav__dropdown--products hidden absolute top-full left-0 bg-white shadow-(--shadow-md) rounded-b-(--radius-md) min-w-[280px] max-h-[400px] overflow-y-auto z-(--z-dropdown)"
               role="menu"
               aria-label="Ürün kategorileri">
            <div class="store-nav__dropdown-header bg-[#f3f4f6] px-4 py-1.5 text-[13px] font-bold text-[#374151]">
              Products
            </div>
            <a href="#" class="store-nav__dropdown-link block px-4 py-2 text-[13px] text-(--store-accent) font-medium hover:bg-[#f3f4f6] transition-colors" role="menuitem">
              See all categories
            </a>
    ...
    if (item.dropdownType === 'company') {
      return `
        <li class="relative">
          ...
          <div class="store-nav__dropdown store-nav__dropdown--company hidden absolute top-full left-0 bg-white shadow-(--shadow-md) rounded-b-(--radius-md) min-w-[200px] z-(--z-dropdown)"
               role="menu"
    ...
    return `
      <li>
        <a href="${item.href}"
           class="store-nav__item ${item.isActive ? 'store-nav__item--active block px-6 py-3 text-(--store-nav-text) text-[14px] font-semibold bg-(--store-nav-active-overlay) transition-colors' : 'block px-5 py-3 text-(--store-nav-text) text-[14px] font-normal transition-colors hover:bg-[rgba(255,255,255,0.1)]'}"
    ...
    <nav id="store-nav" class="store-nav sticky top-0 z-(--z-sticky) bg-(--store-nav-bg) transition-shadow duration-200" aria-label="Mağaza navigasyonu">
      <div class="store-nav__container max-w-(--container-lg) mx-auto px-[clamp(0.75rem,0.5rem+1vw,1.5rem)] lg:px-6 xl:px-8 flex items-center justify-between">
        ...
        <div class="store-nav__search relative hidden xl:block">
          <input type="text"
                 class="store-nav__search-input bg-white rounded-(--radius-sm) px-3 py-1.5 pr-9 text-[13px] text-[#374151] placeholder-(--color-text-muted) w-[200px] border-none outline-none focus:ring-2 focus:ring-white/30"
                 placeholder="${data.searchPlaceholder}"
                 aria-label="Mağaza içi arama" />
          <svg class="store-nav__search-icon absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-(--color-text-muted) cursor-pointer hover:text-(--color-text-secondary) transition-colors"
      ...
      <div id="store-nav-mobile-menu" class="store-nav__mobile-menu hidden xl:hidden bg-(--store-nav-bg) border-t border-white/10">
        ...
        <div class="store-nav__mobile-search px-4 py-3">
          <input type="text"
                 class="store-nav__search-input w-full bg-white rounded-(--radius-sm) px-3 py-2 text-[14px] text-[#374151] placeholder-(--color-text-muted) border-none outline-none"
                 placeholder="${data.searchPlaceholder}"
                 aria-label="Mağaza içi arama" />
        </div>
        <!-- Mobile Menu Items -->
